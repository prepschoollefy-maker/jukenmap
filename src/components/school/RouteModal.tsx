...
export function RouteModal({ school, originLat, originLng, originAddress, onClose }: Props) {
  ...
  const embedUrl = `https://www.google.com/maps/embed/v1/directions?key=${apiKey}&origin=${encodeURIComponent(originAddress || `${originLat},${originLng}`)}&destination=${encodeURIComponent(school.address)}&mode=transit&language=ja`

  const googleMapsUrl = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(originAddress || `${originLat},${originLng}`)}&destination=${encodeURIComponent(school.address)}&travelmode=transit`
